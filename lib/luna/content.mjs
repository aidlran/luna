 ...
 * @returns {Promise<FileBuilder>}
 */
export async function toFile(instance, identityId, value, mediaType = 'application/json') {
  const publicKey = await getPubKey(instance, identityId);
  const metadata = await new FileBuilder()
    .setMediaType('application/json')
    .setValue(cryptOptions({ ...baseCryptOptions, publicKey }), instance);
...
export const put = async (instance, identityId, value, mediaType) =>
  putImmutable(await toFile(instance, identityId, value, mediaType), { instance });
...
export const saveIndex = async (instance, id, newIndex) =>
  putIdentity({
    id,
    instance,
    ref: await put(instance, id, /** @type {never} */ (newIndex ?? (await getIndex(instance, id)))),
  });
